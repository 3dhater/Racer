...
// Keep it on, because it will warn about software rendering (often too slow)
...
//qdbg("QXWindow: chosen pixel format=%d\n",iFormat);
    ...
    if(genFormat&&!genAcc)
    {
      ...
      // Notify user that this is less than favorable
      qwarn("Pixel format (%d) indicates software rendering; may be slow!",
        iFormat);
    } else if(genFormat&&genAcc)
    { //qdbg("%d: Hardware pixel format - MCD",iFormat);
    } else if(genFormat&&!genAcc)
    {
      // Cool, forgot ",iFormat" below which Purify found
      // (evaluation version)
      //qdbg("%d:Hardware pixel format - ICD",iFormat);
    } else
    {
      qwarn("Pixel format (%d) is of unknown type; may be slow"
        " (software rendering)!",iFormat);
    }
  }
#endif
