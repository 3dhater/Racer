 ...
 * 25-03-97: Created!
 * 23-08-99: Node inserts are now reversed for the sake of QInfoIterator.
 * 25-10-99: Support for subtrees to have values
 * 28-03-01: Support for a fallback file in case of missing values
 * 12-10-01: Tree references now possible with '~' (name~prevTree)
 * NOTES:
...
void QInfo::SetFallback(QInfo *info)
// Declare fallback info in case values aren't found in THIS info tree
// Pass 0 to cancel the fallback tree
{
  infoFallback=info;
}
...
//qdbg("ReadToken(cmd=%s)\n",cmd);
    ...
    { // Assign value to node
      if(!lastChild)
      { qwarn("QInfo: assign without symbol\n"); goto skip_let; }
      ReadUptoEOL(f,cmd,sizeof(cmd));
...
//qdbg("Ref tree\n");
      if(!lastChild)
      { qwarn("QInfo: reference without symbol\n"); goto skip_let; }
      ReadUptoEOL(f,cmd,sizeof(cmd));
      refTree=TrackPath(cmd);
      if(!refTree)
      { qwarn("QInfo: reference to unknown tree '%s'",cmd); goto skip_let; }
      if(!refTree->child)
      { qwarn("QInfo: ref to tree '%s' without child",cmd); goto skip_let; }
      // Hang this reference tree as a child to the current node
      lastChild->reference=refTree;
      lastChild->SetType(QINT_TREE);
#ifdef ND_NO_REFS_AS_CHILDREN
      lastChild->child=refTree->child;
      lastChild->SetType(QINT_TREE);
#endif
    } else
    {
 ...
 skip_root:
  ...
  { case QINT_TREE:
      ...
      { indent(fp,level);
        if(node->reference)
        {
          node->reference->GetFullName(fullName);
          if(v)fprintf(fp,"%s~%s=%s\n",n,v);
          else fprintf(fp,"%s~%s\n",n,fullName.cstr());
        } else
        {
          // No reference
...
//qdbg("Try ref %p\n",tree->reference);
   ...
   do_def:
    if(infoFallback)
    {
      // Try fallback parent (may be nested!)
      QFREE(path);
      return infoFallback->GetString(vName,dst,def);
    } else
    {
...
bool QInfo::GetFloats(cstring vName,int n,float *p)
// Shortcut to get multiple values out of 1 string
{ char buf[256],*s;
  int i;
  if(!GetString(vName,buf))
