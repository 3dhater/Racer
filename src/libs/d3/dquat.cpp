...
void DQuaternion::FromEuler(dfloat ex,dfloat ey,dfloat ez)
...
{
  dfloat rx,ry,rz,ti,tj,tk;
  DQuaternion qx,qy,qz;
...
// t=0..1 (1=entirely 'q', 0=entirely this)
{
...
void DQuaternion::ToAxisAngle(DVector3 *axis,dfloat *angle)
// Converts from quaternion to axis-angle. Angle is counterclockwise around
// the axis.
// Code after an example from GDMag, march '98, pp 20. (modified somewhat)
// NOTE: Untested; may also be improved a bit.
{
  dfloat scale,tw;
  tw=dfloat(acos(w))*2.0;
  scale=1.0/(dfloat)sin(tw/2.0);
  axis->x=x*scale;
  axis->y=y*scale;
  axis->z=z*scale;
  // Convert angle of rotation back to degrees
  *angle=(tw*(360.0/2.0))/PI;
}
