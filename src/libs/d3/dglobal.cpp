...
// Globally defined variable for prefs/settings
DGlobal dglobal;
...
int DTexturePool::FindAll(cstring pattern,cbTexturePoolFind cbFunc)
// Finds all textures that match 'pattern' (which can use * and ?).
// Calls cbFunc(DTexture *tex) for every texture found, if cbFunc!=0.
// Returns the number of matching textures found (so just to query whether
// textures with a certain pattern exist, call FindAll(pattern) and
// check if it returned something >0.
// Programmer note: assumes DTP_NO_MAP (no STL map in use)
{
  int i,n;

qdbg("DTexturePool:FindAll(%s)\n",pattern);

  for(i=n=0;i<count;i++)
  {
    if(QMatch(pattern,texName[i]))
    {
