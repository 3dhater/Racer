...
//qdbg("Check pool: '%s' = tex(%p)\n",QFileBase(fname),t);
    ...
    {
      tex[textures]=t;
      // Default is to repeat (bad for skyboxes)
      tex[textures]->SetWrap(DTexture::REPEAT,DTexture::REPEAT);
      // Default is taken from DGlobal prefs
      //tex[textures]->SetWrap(dglobal.prefs.wrapS,dglobal.prefs.wrapT);
...
{char buf[128];
  ...
  img->Write(buf,QBMFORMAT_TGA);
    ...
    // For .BMP images, key out purple (Racer); avoid the operation
    // for other files, since it can be quite costly.
//qdbg("dmaterial; keypurple=%d\n",img->KeyPurple());
    if(strstr(fname,".bmp")||strstr(fname,".BMP"))
    {
//qdbg("Keying on purple in '%s'\n",fname);
      if(img->KeyPurple())
      {
...
//qdbg("DMat:ImportDOF(); MAT0='%s'?\n",buf);
  ...
  while(1)
  {
    ...
    { // Color
      ...
      QPC2Host_FA(&shininess,1);
      // Check for bad DOFs
      if(ambient[3]==0||diffuse[3]==0||specular[3]==0||emission[3]==0)
      {
        static bool fWarned;
        if(!fWarned)
        {
          qwarn("DOF contains alpha=0 for certain colors, which may make"
            " objects transparent!");
          qwarn("This warning will only be issued once.");
          fWarned=TRUE;
        }
        // Correct
