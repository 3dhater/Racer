 ...
 * 16-11-01: AABBTrees are taking over the brute force collisions.
 * NOTES:
 * FUTURE:
 * - The usage of the profiling class RProfile assumes that after
 * track collision detections, time is spent in the physics again.
 * This is probably a safe assumption for the foreseeable future though.
...
* Tree building *
****************/
static void AddGeob(DAABBTree *tree,DGeob *geob)
// Add all geob tris with Racer poly info
// BUGS: May have to copy all bursts (separate materials in 1 geob)
{
  int i,k,n;
  //float v[3][3];
  float *v[3];
  ...
  for(i=0;i<geob->indices/3;i++)
  {
    ...
    rpi=(RTreePolyInfo*)qcalloc(sizeof(RTreePolyInfo));
    // Remember material of this triangle
    rpi->mat=geob->GetMaterialForID(geob->burstMtlID[0]);
    rpi->splineTri=-1;
    pi->SetUserData((void*)rpi);
    tree->AddPoly(pi);
  }
}
static void AddGeode(DAABBTree *tree,DGeode *geode)
{
  int i;
  for(i=0;i<geode->geobs;i++)
    AddGeob(tree,geode->geob[i]);
}
static void AddSplineTris(DAABBTree *tree,RSplineRep *sr)
{
  int i,n=sr->GetTris();
  DAABBPolyInfo *pi;
  RTreePolyInfo *rpi;
  ...
  {
    tri=sr->GetTri(i);
    pi=new DAABBPolyInfo(&tri->v[0].x,&tri->v[1].x,&tri->v[2].x);
    rpi=(RTreePolyInfo*)qcalloc(sizeof(RTreePolyInfo));
    rpi->mat=0;
    rpi->splineTri=i;
    pi->SetUserData((void*)rpi);
...
void RTrackVRML::CreateAABBTree()
// Build an AABB tree from the entire track & splines for faster
// collision detection/determination.
{
  int i;

  trackTree=new DAABBTree();
  for(i=0;i<GetNodes();i++)
  {
    // Only insert colliding/surface nodes
    if(node[i]->flags&(RTV_Node::F_COLLIDE|RTV_Node::F_COLLIDE))
      AddGeode(trackTree,GetNode(i)->geode);
  }
  AddSplineTris(trackTree,GetSplineRep());
  trackTree->BuildTree();
  //trackTree->Save("test.cdt");
}                
...
bool RTrackVRML::AddNode(const RTV_Node *newNode)
{
  ...
  // Calculate model bounding sphere & radius (is done also
  // for each node that enters the culler)
  DBox box;
  nn->geode->GetBoundingBox(&box);
  box.GetCenter(&nn->center);
  nn->radius=box.GetRadius();
...
//qdbg("RTrackVRML:Paint(); create culler\n");
  ...
  for(i=0;i<nodes;i++)
  {
    ...
    // Don't include decal objects, they are drawn last (transparent)
    if(node[i]->flags&RTV_Node::F_DECAL)
      continue;
...
//qdbg("RTrackVRML:Paint(); culler Painted\n",i);
  ...
  //
  // Paint decal objects last (transparent)
  //
  glDisable(GL_LIGHTING);
  glEnable(GL_DEPTH_TEST);
//glDisable(GL_DEPTH_TEST);
...
* LOADING *
**********/
static void cbNoFilter(DTexture *tex)
// Callback function which is called for textures that you want
// no filtering for. For grass for example, filtering is nice and smooth,
// but for roads, lines get blurred out much too soon.
// Probably called from RTrackVRML:Load().
{
qdbg("cbNoFilter(tex=%p)\n",tex);
  // No function to set filtering yet, so just select the texture
  // and change the parameters. Note you must be in the right context though!
  tex->Select();
...
//qdbg("  load VRML stuff\n");
  ...
  // Use texture clamping to avoid seams in horizons for example
  dglobal.prefs.wrapS=GL_CLAMP;
  // Don't clamp T; seems to do too much
...
//qdbg("object '%s'\n",(cstring)s);
    ...
    { 
      ...
      QFile *f;
      // Try normal file first, after that try the archive
      f=new QFile(RFindFile(fileName,trackDir));
      if(!f->IsOpen())
      {
        // Try the archive
        QDELETE(f);
        f=archive->OpenFile(fileName);
        if(!f)
          goto fail;
      }
...
//qdbg("  QDEL ii/infoTrk\n");
  ...
  // Hardcoded trick to turn off filtering for road textures (I think
  // it looks terribly ugly)
  dglobal.texturePool->FindAll("road*",cbNoFilter);

qdbg("RTrackVRML:Build AABB tree\n");
  CreateAABBTree();
...
// Global vars for communication between the callback and GetSurfaceInfo()
static DVector3 *org,*dir;
static RSurfaceInfo *si;
static bool fNonSplineHit;       // Set if a regular triangle was hit

bool cbGetSurfaceInfo(DAABBPolyInfo *pi)
// Gets called through AABB queries.
// Note that 'pi' should point to just 1 polygon, not a list.
{
  RTreePolyInfo *rpi=(RTreePolyInfo*)pi->userData;
  rfloat t,u,v;

//qdbg("cbGSI; plist=%p, rpi=%p\n",pi,rpi);
  // Spline triangle?
  if(rpi->splineTri>=0)
  {
//qdbg("  is spline tri %d\n",rpi->splineTri);
    if(RayTriangleIntersect(&org->x,&dir->x,pi->v[0],pi->v[1],pi->v[2],
      &t,&u,&v))
    {
      // Found intersection with spline tri
      RSplineRep *sr=RMGR->track->GetSplineRep();
      
      // Calculate intersection point (from FindSplineTri())
//qdbg("FST: intersect tri %d at t=%f, u=%f, v=%f\n",i,t,u,v);
      dfloat tSpline,h0,h1;
      // Get distance along spline
      if(rpi->splineTri&1)
      {
        // Every 2nd triangle is ordered so that the U and V coordinate
        // are reversed. A side-effect of the tri building.
        v=1.0f-v;
        u=1.0f-u;
      }

      // Use 3D patch to calculate splined result
      tSpline=((dfloat)(rpi->splineTri/2))+v;
      h0=sr->GetSplineHM(0,0)->GetValue(tSpline);
      h1=sr->GetSplineHM(1,0)->GetValue(tSpline);
      si->x=h0+u*(h1-h0);
      ...
      // Cache this polygon for future intersections
      si->lastPI=pi;

      // Splines have high priority; don't search on
      return FALSE;
    }
  } else
  {
    // Normal track triangle
    if(RayTriangleIntersect(&org->x,&dir->x,pi->v[0],pi->v[1],pi->v[2],
      &t,&u,&v))
    {
      // Found intersection (linearly interpolated, no splines)
      si->x=org->x+t*dir->x;
      si->y=org->y+t*dir->y;
      si->z=org->z+t*dir->z;
//qdbg("Found tree intersect\n");
      // Store misc. poly info
      si->material=rpi->mat;
      if(si->material)
        si->surfaceType=(RSurfaceType*)si->material->GetUserData();
      else
      ...
      // Cache this polygon for future intersections
      si->lastPI=pi;
      // Set a flag; this function may have been called as a cache check,
      // and we want to return a value indicating a triangle is hit,
      // even though we return TRUE. (see GetSurfaceInfo())
      fNonSplineHit=TRUE;

      // Search on (for potential spline tris, which have precedence)
      return TRUE;
    }
  }
...
// Currently returns valid (si->): x,y,z
{
  ...
  RMGR->profile->SwitchTo(RProfile::PROF_CD_WHEEL_SPLINE);
#ifdef LTRACE
qdbg("RTrackVRML:GetSurfaceInfo()\n");
#endif

  if(trackTree!=0&&RMGR->IsDevEnabled(RManager::NO_AABB_TREE)==FALSE)
  {
    // Use AABBTree for more predictive query results (and faster)
    DAABB aabb;

    // Pass ray on in global vars
    ::org=pos;
    ::dir=dir;
    ::si=si;
    ...
    {
      fNonSplineHit=FALSE;
      if(cbGetSurfaceInfo(si->lastPI)==FALSE||fNonSplineHit==TRUE)
      {
        // Still touching
    ...
    si->y=pos->y+dir->y;
    si->z=pos->z+dir->z;
//qdbg("RTrackVRML:GSI; use AABBtree\n");
    aabb.MakeFromRay(pos,dir,1.0f);
    if(si->startNode)
    {
      // Optimized start somewhere down the tree
      si->startNode->Query(&aabb,cbGetSurfaceInfo);
    } else
    { // Just start at the top
      trackTree->Query(&aabb,cbGetSurfaceInfo);
    }

    // No need to do traditional checking
    RMGR->profile->SwitchTo(RProfile::PROF_PHYSICS);
    return;
  }
  ...
  // No spline hits; check regular track geometry
  RMGR->profile->SwitchTo(RProfile::PROF_CD_WHEEL_TRACK);
...
// This function is OBSOLETE since the introduction of the AABBTree,
// which can find potential triangle hits more efficiently.
{
  qerr("CDGeodeTriOBB() is OBSOLETE; use an AABBTree");
#ifdef OBSOLETE_FUNCTION_USE_AABBTREE
  int     g,tri,i,nTris;
  dfloat *pVertex;
...
{
  qerr("RTrackVRML:CollisionOBB() is OBSOLETE; use AABBTree");
#ifdef OBSOLETE_USE_AABB_TREES
  int i;
  RTV_Node *node;
