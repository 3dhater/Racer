...
printf("Q1-3 = %f, %f, %f\n",Q1,Q2,Q3);
printf("A1-3 = %f, %f, %f\n",A1,A2,A3);
printf("B1-3 = %f, %f, %f (fabs)\n",B1,B2,B3);
printf("Box sides: %f x %f x %f\n",box->side[0],box->side[1],box->side[2]);
printf("B1+B2+B3= %f\n",B1+B2+B3);
printf("0.5*(B1+B2+B3)= %f\n",0.5*(B1+B2+B3));
printf("o1->pos=%f %f %f\n",o1->pos[0],o1->pos[1],o1->pos[2]);
*/

  // early exit test
  //dReal depth = plane->p[3] + 0.5*(B1+B2+B3) - dDOT(n,o1->pos);  // ORG
  //dReal depth = plane->p[3] - 0.5*(B1+B2+B3) + dDOT(n,o1->pos);
  dReal depth = -plane->p[3] + 0.5*(B1+B2+B3) - dDOT(n,o1->pos);
/*
printf("n=%f, %f, %f\n",n[0],n[1],n[2]);
printf("n dot o1Pos=%f\n",dDOT(n,o1->pos));
printf("plane d=%f\n",plane->p[3]);
printf("Early depth check: %f\n",depth);
*/
  if (depth < 0) return 0;
...
#undef BAR
  ...
  contact->depth = depth;
//printf("Deepest point: %f, %f, %f\n",p[0],p[1],p[2]);
//printf("Deepest normal: %f, %f, %f\n",n[0],n[1],n[2]);
  if (maxc == 1) return 1;
